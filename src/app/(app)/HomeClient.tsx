...
import type { HomePayload } from "@/lib/api/home-payload";
...
export default function HomeClient({ initial }: { initial: HomePayload }) {
  ...
  const { data: dbWorkouts = [], isLoading, isError, error } = useQuery<DBScheduledWorkout[]>({
    ...
    initialData: initial.scheduledWorkouts as never,
  });

  const { data: dbTemplates = [] } = useQuery<DBWorkoutTemplate[]>({
    queryKey: ["/api/workout-templates"],
    initialData: initial.workoutTemplates as never,
  });
  ...
  const { data: dbRoutineInstances = [] } = useQuery<DBRoutineInstance[]>({
    ...
    initialData: initial.routineInstances as never,
  });
  ...
  return (
    <div className="flex-1 overflow-auto h-full">
      ...
      <div className="mx-auto w-full max-w-xl space-y-6 px-5 pt-3 pb-8 md:max-w-6xl md:px-9 md:pt-7">
        ...
        <GoalsStrip initialSettings={initial.userSettings} initialActiveRoutines={initial.activeRoutineInstances} />
